...
def train(config, model, train_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        metric_test, trigger_f1, argument_f1 = eval(model, test_iter, 'data/result_data/'+str(epoch) + '_test')


        if trigger_F1 < trigger_f1:
            trigger_F1 = trigger_f1
            torch.save(model, "trigger_bilstm_50epoches.pt")
        if argument_F1 < argument_f1:
            argument_F1 = argument_f1
            torch.save(model, "argument_bilstm_50epoches.pt")
