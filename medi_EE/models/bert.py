...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        self.train_path = dataset + '/train_data/example.json'                                # 训练集
...
class Model(nn.Module):
    ...
    def forward(self, x,label,train=True,condidate_entity=None):
        ...
        triggers_y_2d = label
        encoder_out, pooled = self.bert(context, attention_mask=torch.LongTensor(mask).to(self.device).bool(), output_all_encoded_layers=False)
        encoder_out, _ = self.lstm(encoder_out)
        out=self.fc(encoder_out)

        trigger_loss = self.tri_CRF1.neg_log_likelihood_loss(feats=out, mask=torch.ByteTensor(mask).to(self.device).bool(), tags=triggers_y_2d)
